...
function toHaveCSS(locator, nameOrStyles, expectedOrOptions, options) {
  if (typeof nameOrStyles === "string") {
    if (expectedOrOptions === void 0)
      throw new Error(`toHaveCSS expected value must be provided`);
    const propertyName = nameOrStyles;
    const expected = expectedOrOptions;
    ...
    const styles = nameOrStyles;
    const options2 = expectedOrOptions;
    return import_toEqual.toEqual.call(this, "toHaveCSS", locator, "Locator", async (isNot, timeout) => {
      const results = [];
...
function convertStylePropertyNameFromJsToCss(name) {
  const vendorMatch = name.match(/^(Webkit|Moz|Ms|O)([A-Z].*)/);
  if (vendorMatch)
    return `-${(0, import_utils.toKebabCase)(name)}`;
  return (0, import_utils.toKebabCase)(name);
}
